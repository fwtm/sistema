...
@ManagedBean(name = "tipoInvolucradoCDUVController")
@ViewScoped
public class TipoInvolucradoCDUVController implements Serializable {

    private TipoInvolucrado current;
    @EJB
    private ec.gob.digercic.renavi.ejb.TipoInvolucradoFacadeLocal ejbFacade;
    private String tituloPantalla;

    public TipoInvolucradoCDUVController() {
    }
    ...
    public TipoInvolucrado getSelected() {
        if (current == null) {
            current = new TipoInvolucrado();
        }
        return current;
    }
