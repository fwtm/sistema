 ...
 */
@ManagedBean(name = "firmaRenaviController")
@ViewScoped
public class FirmaRenaviControler implements Serializable {
    ...
    public void buscarNacidoVivosNoFirmados() {
        ...
        try {
            if (userLgn.getAgenciaInUser().getCodMsp() != null) {
                ...
                listNacidoVivoPorFirmar = ejbNacidoVivoRenaviFacade.consultarTablaResultList("NacidoVivoRenavi.findInFechaCreacion", parametros);
                ...
                StringBuilder sqlPorVencer = new StringBuilder("select count(*), n.fk_usu_saureg from nacido_vivo_renavi n, madre_renavi m ");
    ...
    public void enviarApplet() throws EntidadException { //Recupero los id's de los registros a los que voy a crear el número de cédula y PDF.
        ...
        if (this.getListSelectNacidoVivo().size() > 0) {
            for (int i = 0; i < this.getListSelectNacidoVivo().size(); i++) {
                temp.append(this.getListSelectNacidoVivo().get(i).getFkCedulMad().getIdMad().toString());
                temp.append("_");
                temp.append(this.getListSelectNacidoVivo().get(i).getNumPartoSistema());
                if (i < this.getListSelectNacidoVivo().size() - 1) {
                    temp.append(";");
                }
            }
