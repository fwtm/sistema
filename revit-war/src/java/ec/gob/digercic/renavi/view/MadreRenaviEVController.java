...
@ManagedBean(name = "madreRenaviEVController")
@ViewScoped
public class MadreRenaviEVController implements Serializable {
    ...
    public MadreRenaviEVController() {
    }
...
//            malformaciones = "ÚNICA";
    ...
    public void iniciar() {
        try {
            ...
            String fechaMad = formatoFecha.format(current.getFechaNacimMad());
            current.setAnioNacimMad(Short.valueOf(fechaMad.substring(6, 10)));
            current.setMesNacimMad(Short.valueOf(fechaMad.substring(3, 5)));
            current.setDiaNacimMad(Short.valueOf(fechaMad.substring(0, 2)));
            ...
            sql.append("'");
            //DFJ
            //Solo si los registros estan firmados
            sql.append(" and n.fk_id_est_fir=2 and n.fk_id_est_reg in (4,8) order by fecha_creacion_nac_viv"); ///DFJ
            try {
            ...
            if (hijoAux.getFotoMad() != null) {
                try {
                    InputStream input = new ByteArrayInputStream(hijoAux.getFotoMad());
            ...
            if (hijoAux.getFkIdNacMad() != null) {
                List<ParametroConsulta> paramsPais = new ArrayList<ParametroConsulta>();
                if (hijoAux.getFkIdNacMad().getIdNac() == 1) {//Ecuatoriana
            ...
            if (hijoAux.getHijosNaciervivSistema() == hijosVivosSistema) {
                ...
                if (hijosAux != null) {
                    if (hijosAux.size() == ((int) hijoAux.getHijosNaciervivSistema())) {
                        ...
                        //DFJ
                        //System.out.println(" ***  " + hijosAux.size() + "  " + hijoAux + "  *****   " + hijoAux.getHijosNaciervivSistema() + "  /// " + hijoAux + "    +++++  " + porEliminarNV);
                        //
    ...
    public List<NacidoVivoRenavi> actualizarDatosNacidos(List<NacidoVivoRenavi> ninios,
        ...
        try {
            ...
            for (NacidoVivoRenavi item : ninios) {
                ...
                temp.setCedulNacViv(item.getCedulNacViv());
    ...
    public List<NacidoVivoRenavi> crearDatosNacidos(List<NacidoVivoRenavi> ninios,
        ...
        try {
            ...
            for (NacidoVivoRenavi item : ninios) {
                ...
                //Reseteo los nombres, apellidos y otros datos
                tempr.setCedulNacViv(item.getCedulNacViv());
...
//            ejbFacadeNacidoVivoRenavi.agregarMalformaciones(hijoAux, malformacionesSeleccionadas);
            ...
            try {
                ...
                RequestContext.getCurrentInstance().execute("PF('statusDialog').hide()");
                JsfUtil.displayMessage(up.getMessage(), JsfUtil.ERROR_MESSAGE);
                RequestContext.getCurrentInstance().update("form_cont:contTab:mssgsBusquedaNV");
            }
    ...
    public static class MadreRenaviControllerConverter implements Converter {
        ...
        public Object getAsObject(FacesContext facesContext, UIComponent component, String value) {
            try {
                ...
                MadreRenaviEVController controller = (MadreRenaviEVController) facesContext.getApplication().getELResolver().
    ...
    public void validarNac() {
        ...
        if (hijosAux.get(0).getFkIdEstReg().getIdEstReg().toString().equals("6") || hijosAux.get(0).getFkIdEstReg().getIdEstReg().toString().equals("7") || hijosAux.get(0).getFkIdEstReg().getIdEstReg().toString().equals("8")) {
            try {
                ...
                if (hijoAnulado != null) {
                    System.out.println("***FECHA NACIMIENTO REGISTRO ANULADO:  " + hijoAnulado.getFechaNacimNacViv() + "  ---  " + hijoAnulado.getIdNacViv());
